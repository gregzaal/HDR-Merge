...
class HDRExecutor:
    ...
    def execute(self):
        ...
        # Calculate total sets for progress tracking from pre-analyzed data
        self.total_sets_global = sum(fd["sets"] for fd in self.folder_data)
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            ...
            for fd in self.folder_data:
                ...
                extension = fd.get("extension", ".tif")
        ...
        notify_phone(
            f"Completed processing folders: {', '.join(pathlib.Path(f).name for f in folders_to_process)}"
        )
...
def execute_hdr_processing(
    ...
    Args:
        folder_data: List of dicts with folder info:
            - path: str, folder path
            - is_raw: bool, whether folder contains RAW files
